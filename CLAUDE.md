...
- Размер: 1080×1920
- Длительность: **равна оригинальному видео** (не обрезать, не зацикливать если не нужно)
- Кодек: H.264, yuv420p, -c:a aac -b:a 192k, -crf 18, -preset fast
...
- Шрифт: auto_font(max_size=70, min_size=44), авто-перенос строк если > MAX_TEXT_W

### ✅ ТЕКУЩИЙ СТАНДАРТ СТИЛИЗАЦИИ ХУКА (вшито навсегда)

- **СТИЛЬ: БЕЗ ПЛАШКИ** — белый текст с чёрной обводкой (stroke_width=4), без фонового прямоугольника
- **РАЗМЕЩЕНИЕ: TOP_ANCHOR = 120px** (сверху, над небом / над пространством)
- **MAX_TEXT_W: 760px**, CENTER_X: 540
- **LINE_GAP: 12px**
- Цвет текста: белый (255, 255, 255, 255), обводка: чёрный (0, 0, 0, 255)
- Всегда рендерить JPG-превью из финального MP4 и показывать перед публикацией

❌ ЗАПРЕЩЕНО: сплошная плашка как основной стиль (только если пользователь явно попросит)

### ✅ АУДИО-СТАНДАРТ (вшито навсегда)

- **Музыка: volume=1.0** (полная громкость)
- **Оригинальный звук видео (интершум): volume=0.15** (тихо фоном)
- **Длина трека = длина видео** (`atrim=0:ДЛИНА_ВИДЕО` — никогда не ставить фиксированное число)
- FFMPEG filter: `[0:a]volume=0.15[a_orig];[music]volume=1.0,atrim=0:N[a_music];amix=inputs=2:duration=first`

❌ ЗАПРЕЩЕНО: обрывать трек раньше чем заканчивается видео
