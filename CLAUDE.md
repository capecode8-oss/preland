...
- Видео: клип из `/footage/` — тихий или без речи
- Аудио при рендере: **ДОБАВИТЬ МУЗЫКУ** из `/music/` (volume 0.8)
- Хук: текстовая плашка поверх видео (SOLID BOX, BOTTOM_ANCHOR 1520px)
- Субтитры: НЕТ

### 🗣️ ГОВОРЯЩАЯ ГОЛОВА (talking head — AI-генерированный персонаж с речью)
- Видео: AI-клип с голосом (Google Flow / Veo 3 с диалогом)
- Аудио при рендере: **ОРИГИНАЛЬНЫЙ ЗВУК ТОЛЬКО** — никакой музыки, никакой замены аудио
- `-map 1:a` из оригинального клипа — не из музыкального файла
- Хук: НЕТ текстовой плашки — первые слова речи = хук
- Субтитры: ДА — Montserrat Black Italic, белый + чёрный контур, уровень груди (y≈700 на 1280px)
- ❌ ЗАПРЕЩЕНО: добавлять музыку поверх голоса, mute оригинального аудио
...
- Видео: показывает хук (3 строки) → зритель останавливается
- Капшен: начинается со **второго хука** — другой угол, продолжение истории, новая любопытность
- Первые ~125 символов капшена = второй стоп-скролл в ленте (то что видно до "...ещё")

**Примеры:**
- Видео: "I ordered coffee on a 9-hour flight. / Felt sick before landing. / A flight attendant told me what's in the water."
- Капшен: "She said none of the crew drink the hot drinks. Not coffee. Not tea. Not the soup." ← НОВЫЙ УГОЛ

❌ ЗАПРЕЩЕНО: начинать капшен той же фразой что на видео
❌ ЗАПРЕЩЕНО: перефразировать хук с видео в первом предложении капшена
✅ ОБЯЗАТЕЛЬНО: второй хук = следующий шаг в истории, открывает новую петлю любопытства
...
```
"Never [УНИВЕРСАЛЬНОЕ ДЕЙСТВИЕ] / Like This [МЕСТО]. / It Can Kill You."
```
- Условие: действие должно быть НОРМАЛЬНЫМ — то что все делают не задумываясь
- Угроза: ФИЗИЧЕСКАЯ (смерть, болезнь, вред телу) — НЕ финансовая
- Примеры залетевших: "Never Sleep Like This On A Plane" (471K наш), план-хак с ремнём (21.3M TikTok)
- Триггеры: T1 SURVIVAL + T4 PATTERN BREAK (визуал норм, текст — опасность)
- ⛔ НЕ работает: финансовые угрозы (USB порты, QR коды, такси) → люди рационализируют "со мной не случится"
...
- Триггеры: T1 SURVIVAL + T3 STATUS ("они знают, ты нет")
...
- Триггеры: T1 + T5 CURIOSITY GAP

**#4 — PATTERN BREAK (визуал + текст-противоречие)**
```
Визуал: человек делает НОРМАЛЬНОЕ, привычное действие
Текст: это ОПАСНО / неправильно / убьёт тебя
```
...
Хук работает ТОЛЬКО если тема:
1. **Универсальная** — касается КАЖДОГО кто летит / едет в отель / на круиз (не "некоторых туристов")
2. **Физическая угроза** — телу, здоровью, жизни (не кошельку)
3. **Неочевидная** — не "не ходи в тёмный переулок", а то что люди делают КАЖДЫЙ РАЗ думая что это безопасно
4. **Инсайдер подтверждает** — доктор / стюардесса / шеф / офицер безопасности знает то что ты не знаешь

### 📊 АЛГОРИТМ ОЦЕНИВАЕТ (в порядке важности):
1. **DM sends** — зритель пересылает другу → главный сигнал виральности
2. **Saves** — сохраняет на потом → второй сигнал
3. **Completion rate** — досмотрел до конца → третий сигнал
4. Лайки и комменты — последние по влиянию
...
5. **Clips mapping (VERIFIED BY VISUAL PREVIEW — не менять без проверки скриншота):**
   ...
   - 🚗 АРЕНДА МАШИНЫ: custom_rental = стойка проката, агент, бумаги на стойке
   - 💳 БАНКОМАТ: custom_atm_1 = руки у ATM, экран выбора валюты | custom_atm_2 = тот же ATM, второй ракурс
   - 🍽️ РЕСТОРАН: custom_restaurant = женщина с банкнотами за столиком, свеча, проверенная скатерть
   - 📄 СТРАХОВКА/ОТЕЛЬ: custom_insurance = женщина в кардигане в номере, смотрит в телефон
   - 🛂 ТАМОЖНЯ/ПАСПОРТ: custom_customs = POV будка пограничника, руки держат ремни
   - 💊 АПТЕКА: custom_pharmacy = женщина читает упаковку в аптеке
...
## 🧠 6 PRIMAL TRIGGERS — ФУНДАМЕНТ КАЖДОГО ХУКА (вшито навсегда)

Каждый хук и капшен должен активировать ОДИН доминирующий биологический триггер:
- **T1 SURVIVAL THREAT** — амигдала: страх, потеря, опасность прямо сейчас
- **T2 SEXUAL SIGNAL** — дофамин: притяжение, forbidden, запрет, красота
- **T3 STATUS THREAT/GAIN** — кортизол/дофамин: "они знают, ты нет" → зависть
- **T4 PATTERN BREAK** — ориентировочный рефлекс: что-то НЕПРАВИЛЬНО в кадре
- **T5 CURIOSITY GAP** — когнитивный зуд: мозг физически не может закрыть луп
- **T6 SOCIAL PROOF SHOCK** — социальное сравнение: "все знают, только ты нет"

Контент НЕ должен быть полезным. Контент должен нажимать на биологическую кнопку за 0.3 секунды до включения логики. Детально — в каждом скилле пайплайна.
...
### 📐 INSTAGRAM REELS SAFE ZONE — ВШИТО НАВСЕГДА (Sep 7 2026)

Canvas: **1080 × 1920px**

| Граница | Пиксели | Почему |
|---------|---------|--------|
| TOP safe limit | y > 270px | Instagram header / status bar |
| BOTTOM safe limit | y < **1520px** | Username, caption, audio bar, action buttons (~400px снизу) |
| LEFT safe limit | x > 108px | 10% от ширины |
| RIGHT safe limit | x < 972px | 10% от ширины |
| **MAX TEXT WIDTH** | **864px** | 972 - 108 |
| **BOTTOM_ANCHOR** | **1520px** | Жёсткий лимит — НЕ МЕНЯТЬ |

⛔ BOTTOM_ANCHOR > 1520px = текст улетает под Instagram UI. Запрещено навсегда.
⛔ Текст шириной > 864px = выходит за safe zone. Запрещено навсегда.

**SOLID BOX placement (сплошная плашка):**
- Один прямоугольник на весь текст (не отдельные плашки на каждую строку)
- `BOTTOM_ANCHOR = 1520px` (жёсткий лимит Instagram safe zone)
...
## 🧬 НЕЙРО-ПРАВИЛА ХУКА — 6 ЗАКОНОВ (Sep 2026, research-backed — вшито навсегда)

**Каждый хук проверяется по всем 6 законам. Один провал = переписать с нуля.**

**ЗАКОН 1 — AMYGDALA HIJACK:** Первые 3 СЛОВА должны содержать физическую угрозу, акт нарушения или потерю. Амигдала срабатывает за 0.3 секунды до включения логики. Если слова 1-3 не содержат сигнал угрозы — хук мёртв до начала.

**ЗАКОН 2 — LOSS FRAMING (2.5× сильнее gain):** Всегда фреймировать как уже происходящую потерю. "Your card details left with it." — не "here's how to protect your card." Потеря должна быть совершённой, не потенциальной.

**ЗАКОН 3 — INFORMATION GAP = ФИЗИЧЕСКАЯ БОЛЬ:** Открыть когнитивный луп — никогда не закрывать его в хуке. Мозг буквально испытывает дискомфорт от незакрытого вопроса. Триггер-слова: "without knowing" / "I saw it on camera" / "I did not pack it." Луп открывается в строке 2-3 и остаётся открытым до капшена.

**ЗАКОН 4 — PREDICTION VIOLATION:** Нормальный визуал + угрожающий текст = самый мощный pattern break. Клип выглядит как обычная сцена. Текст говорит что это опасно. Мозг получает противоречие и не может игнорировать.

**ЗАКОН 5 — IDENTITY THREAT:** Угроза идентичности активирует амигдалу идентично физической угрозе. "They chose me before I boarded the metro." — "выбрали меня" = identity threat. "You scanned it at dinner." — "ты сделал это" = личная причастность, не абстракция.

**ЗАКОН 6 — STAKES COMPRESSION:** Весь кино-момент в 3 строках: экспозиция → событие → необратимое последствие. Зритель ПРОЖИВАЕТ ситуацию, не узнаёт о ней. "He grabbed my wrist. / I could not pull away. / This is how it starts." = зритель внутри сцены.
