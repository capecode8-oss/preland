---
name: kira-hooks
description: Write viral Instagram Reel hooks for @thekiramethod in competitor style (maks.motivator format) AND enforce face clearance on every clip. Runs a full production team audit before finalizing any hook. Use for every reel — no exceptions.
---

# KIRA Hook Production System
## Competitor Format: maks.motivator | Niche: Travel · Life Hacks · Food · Health · Money

---

## 🔴 REPTILIAN BRAIN LAW — ЗАКОН #1 (вшито навсегда, выше всего остального)

**Контент должен быть НЕ ПОЛЕЗНЫМ — а НЕИЗБЕЖНЫМ.**

Не "интересно". Не "узнал что-то новое". А — **невозможно не смотреть**. Физически.

Рептильный мозг (R-комплекс) управляет выживанием. Он не читает, не анализирует. Он сканирует за 0.3 секунды и даёт один сигнал: **ОПАСНОСТЬ / УГРОЗА / ТАЙНА КОТОРУЮ ТЫ УПУСТИЛ**.

**Золотое правило:** Зритель должен почувствовать что-то В ЖИВОТЕ — до того как включился мозг. Не "это интересно" — а "ПОДОЖДИ. ЧТО." Не интеллектуальное любопытство — а физический зуд. Инстинкт.

**Контент который останавливает скролл за 1 секунду — всегда бьёт в одно из трёх:**
- **УГРОЗА ресурсу** (деньги/здоровье/безопасность теряются ПРЯМО СЕЙЧАС)
- **ПРЕДАТЕЛЬСТВО** (кому-то ты доверял — тебя обманывали)
- **ЗУУДЯЩАЯ ТАЙНА** (все вокруг уже знают — и только ты нет)

**Тест перед публикацией:** прочитай первую строку. Зритель почувствовал что-то в животе? Или просто "интересно"? Если "интересно" — хук слабый. Переписать.
...
1. **Бить в рептильный мозг** — та часть мозга, которая реагирует раньше чем человек успел подумать. Страх потери. Опасность. Что-то личное. Деньги. Здоровье. Срабатывает за 0.3 секунды. Зритель должен почувствовать в животе — ДО включения логики. Не "интересно" — а физический зуд, почти дискомфорт. Инстинкт выживания.
...
5. **Любопытство как крючок** — дать вопрос без ответа. Показать что что-то не так. Намекнуть на тайну которую все остальные уже знают — а он нет. Зуд должен быть невыносимым — зритель думает об этом даже после свайпа.
...
**Без этих 6 пунктов — хук не выходит. Никогда.**

**ГЛАВНАЯ ПРОВЕРКА КОМАНДЫ:** "Этот хук вызывает физическую реакцию — или просто интеллектуальный интерес?" Если только интерес — хук слабый. Рептильный мозг молчит. Переписать.
...
- Third person "She/He did X" — FORBIDDEN
- Generic benefit hooks ("Here's how to improve your X") — WEAK
...
## TOPIC BANK — rotate, never repeat same topic in 3 days
...
- Towel/minibar dispute (one email / one call kills the charge)
- Hotel room upgrade hack (ask at 3pm, not check-in)
- Safe cleaning frequency (it's not cleaned between guests)
- Ice machine floor location (always middle floors, reason is unsettling)
...
- Blue Zone breakfast (Sardinia/Okinawa longevity morning meal)
- Earbuds + street noise = 3× hearing loss risk (doctor research)
- Inflammation markers: common kitchen items spiking them
- Olive oil fraud (most "extra virgin" bottles fail purity tests)
- Supermarket perimeter rule (real food vs processed food layout)
- Coffee timing mistake (cortisol window, most drink it at worst time)

### 💰 MONEY TRAPS
- Rental car insurance: credit card already covers it (most people pay twice)
- Airport lounge day pass = $40 (vs $25 sandwich + chair)
...
**КАК ВЫБРАТЬ ФОРМАТ — команда решает сама, без подсказки пользователя:**
...
- ✅ "( I was sick for 3 days straight ↓ )" — личная потеря, конкретно
- ✅ "( my friend lost $14,000 over this ↓ )" — цифра + последствие
- ✅ "( one reply email fixed it in 5 min ↓ )" — намёк на решение
- ❌ "( watch till the end ↓ )" — абстрактно, слабо
- ❌ "( you need to know this ↓ )" — ничего не говорит
- ❌ "( see below ↓ )" — мёртвый badge

**Пользователь НЕ должен говорить нужен badge или нет. Команда решает сама по этому алгоритму.**
...
⚠️ **Текст ВСЕГДА внизу — в ЗЕЛЁНОЙ ЗОНЕ (нижняя часть кадра). Лицо всегда свободно сверху.**

**ЗЕЛЁНАЯ ЗОНА:** y = 1400–1820px (нижние ~25% кадра 1080×1920)

**Правило расчёта:**
- `BOTTOM_ANCHOR = 1820` (100px от нижнего края)
- `hook_y0 = BOTTOM_ANCHOR - total_text_block_height`
- total_text_block_height = сумма всех плашек (строки + CTA) + BOX_GAP между ними

- ❌ НЕ ставить текст сверху (hook_y0 = 80 — ЗАПРЕЩЕНО)
- ❌ НЕ центрировать вертикально
- ❌ НЕ делать body clearance анализ, не извлекать кадры
- ✅ Всегда: текст внизу в зелёной зоне, cta_bottom ≤ 1820px
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📝 CAPTION (~1800 символов — вставить под рилсом):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[TITLE — одна жирная строка, сильнее хука, не повторяет его]
([SUBTITLE — что узнаешь / мост к CTA, в скобках])

[SECOND HOOK — 1-2 строки, открывает НОВЫЙ луп]

[CONTEXT — 2-3 предложения — почему это важно, что большинство не знает]

[DETAIL — конкретный факт/цифра/история]

[PAYOFF — что делать / что на самом деле происходит]

[CTA — 3 секунды = понятно: ЧТО (free one-page card) + ЧТО ДАЁТ (4 steps for 3AM) + КАК (comment CALM / link in bio)]

[SAVE LINE]

#[5-8 релевантных хештегов]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Длина: ~1800 символов | Язык: как iMessage другу в 11 вечера | Предложения ≤15 слов
```
...
### ПРАВИЛО КАПШЕНА — вшито навсегда

Структура всегда: Second Hook → Context → Detail → Payoff → CTA → Save Line  
Длина: ~1800 символов (не меньше 1700, не больше 1900)  
...
```
ЗЕЛЁНАЯ ЗОНА: текст внизу | hook_y0 = BOTTOM_ANCHOR(1820) - total_block_height
QA: center=540 | box_w=XXX | hook_y0≥1400 | cta_bottom≤1820 ✅
```

### Step 4 — Render + JPG preview

Always show JPG preview frame from final MP4 before scheduling. No exceptions.
